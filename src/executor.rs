    ...
    pub fn execute(&self) -> Result<(), &'static str> {
        return match &self.config.command {
            ...
            Commands::Spam(args, press_keys, bind_keys) => {
                ...
                let mut file = File::create(format!("./macros/{}.ahk", self.config.filename.unwrap())).unwrap();
                file.write_all(AHK_HEADER.as_bytes()).unwrap();

                let mut ahk_bind_keys: String = String::from("");
                for key in bind_keys {
                    ahk_bind_keys = format!("{}{} & ", ahk_bind_keys, key);
                }
                ...
                let mut ahk_press_keys: String = String::from("");
                for key in press_keys {
                    ahk_press_keys.push_str("Send, ");
                    ahk_press_keys.push_str(key);
                    ahk_press_keys.push_str("\n");
                }
                ahk_press_keys.push_str("Return");
